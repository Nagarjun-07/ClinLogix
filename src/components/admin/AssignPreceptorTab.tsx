...
interface Profile {
  ...
  role: string;
  isPending?: boolean;
  student_count?: number; // Add these fields
  max_students?: number;
}
...
export function AssignPreceptorTab() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // 1. Fetch Students (Profiles) with auto-created valid UUIDs
      const { data: sData } = await supabase.from('profiles').select('*').eq('role', 'student');
      if (sData) setStudents(sData);

      // 2. Fetch Preceptors with STATS
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              >
                ...
                {preceptors.map(p => {
                  const isFull = (p.student_count || 0) >= 5;
                  return (
                    <option key={p.id} value={p.id} disabled={isFull}>
                      {p.full_name} ({p.email}) - {p.student_count || 0}/5 Assigned {isFull ? '(Full)' : ''}
                    </option>
                  );
                })}
              </select>
